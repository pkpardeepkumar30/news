...
function badges(story){
  ...
  return `<div class="image-badges">${story.demo?'<span class="badge demo">Demo</span>':''}${sourceTypes.has('social')?'<span class="badge social">Social source</span>':''}${sourceTypes.has('government')?'<span class="badge official">Official source</span>':''}<span class="badge">${escapeHtml(story.evidence_status)}</span></div>`;
}
