...
function renderLatest(){
  ...
  el('leadLayout').innerHTML = `<article class="lead-card" data-story="${lead.id}">${image(lead)}<div class="lead-copy"><span class="kicker">${escapeHtml(lead.category)} · Underreported ${lead.underreported_score}/100</span><h3 class="${headlineClass(lead.title)}">${escapeHtml(lead.title)}</h3><p class="story-summary">${escapeHtml(lead.summary)}</p>${meta(lead)}</div></article><aside class="side-list"><div class="side-list-header"><strong>Latest developments</strong><span>${side.length}</span></div>${side.map(s=>`<article class="side-story" data-story="${s.id}"><img src="${escapeHtml(s.image.url)}" alt="" loading="lazy" onerror="this.src='assets/images/fallback.svg'"><div><span class="kicker">${escapeHtml(s.category)}</span><h4>${escapeHtml(s.title)}</h4><p>${escapeHtml(s.location)} · ${shortDate(s.updated_at)}</p></div></article>`).join('')}</aside>`;
...
function storyCard(s){ return `<article class="news-card" data-story="${s.id}">${image(s)}<div class="news-copy"><span class="kicker">${escapeHtml(s.location)}</span><h3>${escapeHtml(s.title)}</h3><p class="story-summary">${escapeHtml(s.summary)}</p><div class="card-footer"><span>${escapeHtml(s.sources[0]?.name||'Source pending')}</span><span class="confidence ${s.confidence.level.toLowerCase()}">${s.confidence.level}</span></div></div></article>`; }
...
function renderArchive(){
  ...
  const update=async()=>{
    ...
    el('archiveResults').innerHTML=Object.entries(days).sort((a,b)=>b[0].localeCompare(a[0])).map(([day,items])=>`<section class="archive-day"><h3>${new Intl.DateTimeFormat('en-IN',{weekday:'long',day:'numeric',month:'long',year:'numeric'}).format(new Date(day))}</h3>${items.map(s=>`<article class="archive-item" data-story="${s.id}"><img src="${escapeHtml(s.image.url)}" alt=""><div><h4>${escapeHtml(s.title)}</h4><p>${escapeHtml(s.category)} · ${escapeHtml(s.location)} · ${escapeHtml(s.evidence_status)}</p></div><span class="confidence ${s.confidence.level.toLowerCase()}">${s.confidence.level}</span></article>`).join('')}</section>`).join('')||'<div class="empty-state">No stories in this archive selection.</div>';
